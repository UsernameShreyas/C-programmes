/* Q3) There is a company NI Infotech which has N number of employees working in it.
...
struct employee
{
    int basic;          //Defining Variables
    int experience;     //Defining Variables
    float salary;       //Defining Variables
}

employee[50];

void read_employee(struct employee[] ,int);     //Function Call 
void calculate_salary(struct employee[] , int);     //Function Call

int main()
{
    int n;

    printf("Enter the total number of employees in the company : \n");
    scanf("%d",&n);
    read_employee(employee,n);

    return 0;
}
void read_employee(struct employee employee[50],int b)      //Function Definition   
{
    int i=1;
    int j = b;
...
void calculate_salary(struct employee employee[50], int b)      //Function Definition
{
    ...
    if(employee -> experience >= 5 || employee -> experience <= 7)
    {
        ...
        printf("\n\n Basic : %d\n Total Salary after adding Bonus: %.2f \n",employee->basic,employee->salary);
    }
